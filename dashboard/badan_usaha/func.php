...
function GetByStatus(){
  $status = "Menunggu";
  $query = "SELECT * FROM badan_usaha WHERE status='$status'";
  $exe = mysqli_query(Connect(),$query);
  while($data = mysqli_fetch_array($exe)){
    $datas[] = array('id' => $data['id'],
		'nama_usaha' => $data['nama_usaha'],
...
$exe = mysqli_query(Connect(),$query);
  ...
  echo '<script>window.location = "?r=badan_usaha/index"</script>';
  }
  else{
...
$exe = mysqli_query(Connect(),$query);
  if($exe){
    ...
    echo '
    <script>
    window.location = "?r=badan_usaha/index"
    </script>'; 
  }
  else{
...
function Verifikasi($id){
  ...
  $query = "UPDATE `badan_usaha` SET `status` = '$status' WHERE  `id` =  '$id'";
$exe = mysqli_query(Connect(),$query);
  if($exe){
    // kalau berhasil
    $_SESSION['message'] = " Data Sudah diubah ";
